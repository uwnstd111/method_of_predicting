...
def retHoltValues(df):
    ...
    for gamma in range(0, 99):
        for alpha in range(0, 99):
            ...
            for i in range(1, len(db)):
                temp_pt = ((alpha + 1) * 0.01) * db[i][0] + (1 - ((alpha + 1) * 0.01)) * (pt + bt)
                bt = ((gamma + 1) * 0.01) * (temp_pt - pt) + (1 - ((gamma + 1) * 0.01)) * bt
                pt = temp_pt
    ...
    optimal_alpha = (optimal_alpha + 1) * 0.01
    optimal_gamma = (optimal_gamma + 1) * 0.01
